 ...
 * (b) Runtime: ReadOnlyGraphHandleImpl.write() throws SecurityException
 * (c) Scope UUID orthogonality: scope_id held by a handle is never mutated by context-size ops
 */

import { describe, it, expect } from 'vitest';
import type { ReadOnlyGraphHandle } from '../../packages/workers/src/base/read-only-handle.js';
import { ReadOnlyGraphHandleImpl, SecurityException } from '../../packages/workers/src/base/read-only-handle.js';
...
describe('Tool boundary — ReadOnlyGraphHandle interface', () => {
  it('ReadOnlyGraphHandle interface does NOT declare write() — compile error if called', () => {
    ...
    const roHandle: ReadOnlyGraphHandle = new ReadOnlyGraphHandleImpl('scope-uuid-1', {} as never);
  ...
  it('ReadOnlyGraphHandleImpl.write() throws SecurityException at runtime', () => {
    const roHandle = new ReadOnlyGraphHandleImpl('scope-uuid-2', {} as never);
  ...
  it('Scope UUID is orthogonal to context-size ops (REQ-23, ADR 33)', () => {
    ...
    const roHandle = new ReadOnlyGraphHandleImpl(scopeId, {} as never);
