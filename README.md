...
"Learns from repetition" is an empirical claim, so we measured it causally rather than asserting it. On a microservice-setup task with hidden dependency ordering, a cold agent makes a non-obvious mistake once; after the loop crystallizes the *corrected* order, later runs avoid it and events-to-convergence drops from 26 to 24 and holds. The effect grows with genuinely hidden structure (12% on an 18-step variant), and a naturally-occurring precondition (install a skill's CLI before use) is learned robustly. The benchmark is honest about limits too: the deep 18-step task is *bimodally* variance-fragile on a non-deterministic model — it collapses about half the time at baseline (the loop's first crystallization is sometimes wrong and then recalled forever, the "error avalanche" of self-improving systems). That diagnosis led to a **restoring force**: a per-crystallization trust substrate that softens an ingredient only when the run actually followed its rules (a deterministic conformance check, not a second LLM), retires the provably-stale, and keeps trust recency-weighted. The mechanism provably fires — the metabolism was observed live retiring a bad runbook and letting the loop escape the collapse attractor — but, measured under (still-thin) sampling, it does **not yet** robustly lower the collapse-rate: retirement is lagged, and collapses caused by the model failing to follow a *correct* runbook are composition errors the substrate deliberately leaves alone. The statistical regression gate (`npm run eval:loop`) caught that over-claim where a single curve would have hidden it — which is exactly why a self-improving loop cannot be guarded by unit tests (§5.8).
