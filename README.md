...
"Learns from repetition" is an empirical claim, so we measured it causally rather than asserting it. On a microservice-setup task with hidden dependency ordering, a cold agent makes a non-obvious mistake once; after the loop crystallizes the *corrected* order, later runs avoid it and events-to-convergence drops from 26 to 24 and holds. The effect grows with genuinely hidden structure (12% on an 18-step variant), and a naturally-occurring precondition (install a skill's CLI before use) is learned robustly. The benchmark is honest about limits too: template consolidation on the deep 18-step task is variance-fragile on a non-deterministic model, an open problem documented in full. The same harness surfaced and fixed three latent defects, and shipped a statistical regression gate (`npm run eval:loop`) because a self-improving loop cannot be guarded by unit tests.
