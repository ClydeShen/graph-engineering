...
var Sdk = class {
				...
				this.functions.set(functionId, {
					...
					handler: async (input, traceparent, baggage) => {
						...
						if ((0, _iii_dev_observability_internal.getTracer)()) {
							...
							return _opentelemetry_api.context.with(parentContext, () => (0, _iii_dev_observability.withSpan)(`call ${functionId}`, { kind: _opentelemetry_api.SpanKind.SERVER }, async () => await runHandler()));
						}
	...
	*/
	...
	onTriggerRegistrationResult(message) {
		if (!message.error) return;
		const triggerType = message.trigger_type ?? message.type ?? "";
		console.error(`[iii] Trigger registration failed for "${message.id}" (${triggerType}): ${message.error.message}`);
	}
