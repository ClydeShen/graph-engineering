...
describe('McpClientWorker', () => {
  ...
  it('is a no-op when MCP_SERVER_URLS is not set', async () => {
    ...
    const worker = new McpClientWorker(mockPool);
  ...
  it('calls the tool and writes result to calling scope via occWrite', async () => {
    process.env['MCP_SERVER_URLS'] = 'http://localhost:3001';
    const { McpClientWorker } = await import('./mcp-client.worker.js');
    const worker = new McpClientWorker(mockPool);
    ...
    expect(mockOccWrite).toHaveBeenCalledWith(mockPool, {
      scopeId: scope_id,
      entityId: entity_id,
